...
class UpdatedDateTimePicker(DateTimeInput):
    class Media:
        class JsFiles(object):
            def __iter__(self):
                yield 'bower_components/moment/min/moment.min.js'
                yield 'bower_components/eonasdan-bootstrap-datetimepicker/src/js/bootstrap-datetimepicker.js'


        js = JsFiles()
        css = {'all': ('bower_components/eonasdan-bootstrap-datetimepicker/build/css/bootstrap-datetimepicker.min.css',), }
    ...
    def render(self, name, value, attrs=None):
        ...
        input_attrs = dict(
            [(key, conditional_escape(val)) for key, val in input_attrs.items()])  # python2.6 compatible
